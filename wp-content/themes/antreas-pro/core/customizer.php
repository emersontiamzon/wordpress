		...
		foreach ( $optionsets as $current_language => $current_language_name ) {
			...
			switch ( $args['type'] ) {
				...
				case 'image':
					...
					break;
				case 'collection':
					$customize->add_control( new CPO_Customize_Collection_Control( $customize, 'antreas_' . $control_id, $args ) );
					break;
				case 'checkbox':
					$args['type'] = 'epsilon-toggle';
					$customize->add_control( new Epsilon_Control_Toggle( $customize, 'antreas_' . $control_id, $args ) );
					break;
				case 'dimensions':
