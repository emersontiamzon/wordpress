...
if ( ! function_exists( 'antreas_setup' ) ) {
	...
	function antreas_setup() {
		...
		if ( ! defined( 'ANTREAS_CORE_URL' ) ) {
			define( 'ANTREAS_CORE_URL', get_template_directory_uri() . '/core' );
		}
		...
		$settings = get_option( 'antreas_settings' );
		if ( isset( $settings['general_css'] ) ) {
			wp_update_custom_css_post( $settings['general_css'] );
			unset( $settings['general_css'] );
			update_option( 'antreas_settings', $settings );
		}
...
if ( ! function_exists( 'antreas_scripts_front' ) ) {
	...
	function antreas_scripts_front() {
		...
		wp_register_script( 'antreas_stellar', ANTREAS_ASSETS_JS . 'jquery-stellar.js', array( 'jquery' ), ANTREAS_VERSION, true );
		...
		wp_register_script( 'antreas-waypoints', ANTREAS_ASSETS_JS . 'jquery.waypoints.js', array( 'jquery' ), ANTREAS_VERSION, true );

		$slider_effect = antreas_get_option( 'slider_effect' );
		switch ( $slider_effect ) :
			case 'flipHorz':
			case 'flipVert':
				wp_enqueue_script( 'cycle2-flip', ANTREAS_ASSETS_JS . 'cycle2-flip.min.js', array( 'jquery', 'antreas_cycle' ), ANTREAS_VERSION, true );
				break;
			case 'tileSlide':
			case 'tileBlind':
				wp_enqueue_script( 'cycle2-tile', ANTREAS_ASSETS_JS . 'cycle2-tile.min.js', array( 'jquery', 'antreas_cycle' ), ANTREAS_VERSION, true );
				break;
			case 'shuffle':
				wp_enqueue_script( 'cycle2-shuffle', ANTREAS_ASSETS_JS . 'cycle2-shuffle.min.js', array( 'jquery', 'antreas_cycle' ), ANTREAS_VERSION, true );
				break;
		endswitch;

		if ( antreas_get_option( 'enable_animations' ) === true ) {
			wp_enqueue_script( 'antreas-waypoints' );
		}
...
if ( ! function_exists( 'antreas_scripts_back' ) ) {
	...
	function antreas_scripts_back() {
		...
		wp_enqueue_script( 'antreas_customizer', ANTREAS_ASSETS_JS . 'customizer.js', array(), ANTREAS_VERSION );
		...
		if ( defined( 'CPOTHEME_USE_PORTFOLIO' ) && CPOTHEME_USE_PORTFOLIO == true && $screen->post_type == 'cpo_portfolio' ) {
			wp_enqueue_script( 'antreas_script_gallery', ANTREAS_ASSETS_JS . 'gallery.js', array(), ANTREAS_VERSION );
		}
	}
}
...
if ( ! function_exists( 'antreas_add_styles' ) ) {
	...
	function antreas_add_styles() {
		...
		wp_register_style( 'antreas-prettyphoto', ANTREAS_ASSETS_CSS . 'prettyphoto.css', array(), ANTREAS_VERSION );
		...
		wp_add_inline_style( 'antreas-main', antreas_generate_custom_css() );

		// Font Libraries.
		wp_register_style( 'antreas-linearicons', ANTREAS_ASSETS_CSS . 'icon-linearicons.css', array(), ANTREAS_VERSION );
		wp_register_style( 'antreas-typicons', ANTREAS_ASSETS_CSS . 'icon-typicons.css', array(), ANTREAS_VERSION );
	}
}
...
if ( ! function_exists( 'antreas_add_admin_styles' ) ) {
	...
	function antreas_add_admin_styles() {
		...
		wp_enqueue_style( 'antreas-linearicons', ANTREAS_ASSETS_CSS . 'icon-linearicons.css', array(), ANTREAS_VERSION );
		wp_enqueue_style( 'antreas-typicons', ANTREAS_ASSETS_CSS . 'icon-typicons.css', array(), ANTREAS_VERSION );
		wp_enqueue_style( 'thickbox' );
		wp_enqueue_style( 'antreas-select2-css', ANTREAS_ASSETS_CSS . 'select2.min.css', array(), ANTREAS_VERSION );
...
require_once $core_path . 'classes/class-antreas-customize-sortable-control.php';
