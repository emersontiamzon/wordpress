...
if ( ! function_exists( 'antreas_user_fields' ) ) {
	function antreas_user_fields( $user, $metadata, $title = '' ) {
		if ( empty( $metadata ) ) {
			return;
		}
		$output = '';
		...
		if ( ! empty( $metadata ) && is_array( $metadata ) ) {
			foreach ( $metadata as $current_meta ) {
				...
				if ( $field_type == 'readonly' ) {
					...
					$output .= antreas_form_upload( $field_name, $field_value, null, $user );
...
if ( ! function_exists( 'antreas_user_fields_save' ) ) {
	function antreas_user_fields_save( $user_id, $metadata ) {
