...
class Antreas_Customize_ContactForm_Control extends WP_Customize_Control {
	...
	public function render_content() {
		...
		<?php if ( ! $this->is_kaliforms_active() ) : ?>
			<p><?php _e( 'There are no contact form plugins activated. Please activate Kali Forms.', 'cpotheme' ); ?></p>
		<?php else: ?>
			<?php $forms = $this->get_kaliforms(); ?>
			<div class="cpotheme_contact_control__<?php echo $plugin_data['slug']; ?>">
				<?php if ( ! empty( $forms ) ) : ?>
					<span class="customize-control-title"><?php _e( 'Select a contact form', 'cpotheme' ); ?></span>
					<select>
						<option value="default"><?php _e( 'Select form', 'cpotheme' ); ?></option>
