...
class Epsilon_Welcome_Screen {
	...
	public function __construct( $config = array() ) {
		...
		require_once 'inc/class-edd-theme-helper.php';
		require_once 'inc/class-epsilon-updater-class.php';

		$theme    = wp_get_theme();
		$defaults = array(
			'theme-name'  => $theme->get( 'Name' ),
			'theme-slug'  => $theme->get( 'TextDomain' ),
			'author-logo' => get_template_directory_uri() . '/includes/libraries/welcome-screen/img/macho-themes-logo.png',
			'actions'     => array(),
			'plugins'     => array(),
			'notice'      => '',
			'sections'    => array(),
	...
	private function add_default_options() {
		if ( ! empty( $this->actions ) ) {
			...
			if ( empty( $actions_left ) ) {
				...
				update_option( $this->theme_slug . '_actions_left', $actions_left );
			}
		}
	...
	public static function import_demo_content() {
		...
		require_once get_template_directory() . '/includes/libraries/welcome-screen/inc/class-regina-theme-importer.php';
		$importer = new Regina_Theme_Importer();
