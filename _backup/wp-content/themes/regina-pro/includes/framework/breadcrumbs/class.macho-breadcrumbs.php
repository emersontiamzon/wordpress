...
class Macho_Breadcrumbs {
	...
	public function __construct() {
		...
		$defaults = array(
			...
			'show_terms'             => get_theme_mod( 'regina_blog_breadcrumb_menu_post_category', 1 ),
			'home_label'             => __( 'Home', 'regina' ),
			'tag_archive_prefix'     => __( 'Tag:', 'regina' ),
			'search_prefix'          => __( 'Search:', 'regina' ),
			'error_prefix'           => __( '404 - Page not Found', 'regina' ),
		);

		// Setup a filter for changeable variables and meger it with the defaults
		$args     = apply_filters( 'macho_breadcrumbs_defaults', $defaults );
	...
	private function wrap_breadcrumbs() {
		$this->html_markup = sprintf( '<div id="breadcrumb" class="mt-breadcrumbs"><ul class="hidden-xs">%s</ul></div>', $this->html_markup );
	}
	...
	private function get_breadcrumb_home() {
		...
		if ( ! is_front_page() ) {
			...
			$home_link = $this->get_single_breadcrumb_markup( __( 'Blog', 'regina' ) );
		}
	...
	private function get_single_breadcrumb_markup( $title, $link = '', $separator = true, $microdata = true ) {
		...
		// if ( $separator ) {
		// 	$separator_markup = sprintf( '<span class="mt-breadcrumb-sep">%s</span>', $this->separator );
		// }
