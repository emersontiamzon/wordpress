...
add_action('macho/options/field/background/render', array('Macho_Background_Field', 'render'), 1, 2);
add_action('macho/options/field/background/schema', array('Macho_Background_Field', 'schema'), 1, 2);
add_action('macho/options/field/number/render', array('Macho_Number_Field', 'render'), 1, 2);
add_action('macho/options/field/background/enqueue', array('Macho_Background_Field', 'enqueue'));

/**
 * Macho_Background_Field simple select field.
 */
class Macho_Background_Field extends Macho_Field {
    ...
    public static function render($data = array(), $object){
        ...
        echo '<table>';
            echo '<tr>';
                ...
                echo '</td>';
                echo '<td width="33%">';
                    //echo '<label><input type="checkbox" name="'.$data['option_name'].'['.$data['name'].'][transparent]" id="'.$data['id'].'-transparent" value="1"'.((isset($data['value']['transparent']) && $data['value']['transparent'] == true) ? ' checked="checked"' : '').' /> '.__('Transparent', 'macho').'</label>';
                    echo '<label>';
                    echo '<input type="number" name="'.$data['option_name'].'['.$data['name'].'][transparent]" id="'.$data['id'].'-transparent" value="'.((isset($data['value']['transparent'])) ? $data['value']['transparent'] : '').'" step="'.$data['step'].'" min="'.$data['min'].'" max="'.$data['max'].'" autocomplete="false" />';
                    echo '</label>';
                echo '</td>';
                echo '<td width="33%">';
                    //media field
        ...
        /*
            echo '<tr class="image"'.(($data['value']['background-image']) ? '' : ' style="display: none;"').'>';
                echo '<td width="50%">';
                    echo '<select name="'.$data['option_name'].'['.$data['name'].'][background-attachment]" id="'.$data['id'].'-background-attachment" class="selectize" placeholder="'.__('Background Attachment', 'macho').'">';
                ...
                echo '</td>';
                echo '<td width="50%">';
                    echo '<select name="'.$data['option_name'].'['.$data['name'].'][background-position]" id="'.$data['id'].'-background-position" class="selectize" placeholder="'.__('Background Position', 'macho').'">';
            ...
            echo '<tr class="image"'.(($data['value']['background-image']) ? '' : ' style="display: none;"').'>';
                echo '<td width="50%">';
                    echo '<select name="'.$data['option_name'].'['.$data['name'].'][background-size]" id="'.$data['id'].'-background-size" class="selectize" placeholder="'.__('Background Size', 'macho').'">';
                ...
                echo '</td>';
                echo '<td width="50%">';
                    echo '<select name="'.$data['option_name'].'['.$data['name'].'][background-repeat]" id="'.$data['id'].'-background-repeat" class="selectize" placeholder="'.__('Background Repeat', 'macho').'">';
